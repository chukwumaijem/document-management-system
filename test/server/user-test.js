...
describe('User Tests', function () {
  ...
  describe('Login User', function () {
    ...
    it('should throw an error if username is not provided', function (done) {
    ...
    it('should throw an error if username or password is invalid', function (done) {
      api.post('/users/login').send({ username: 'ebuka', password: 'wrongpassword' })
        .expect(400).end((err, res) => {
          if (err) {
            return done(err);
          }
          expect(res.body).to.have.property('error');
          expect(res.body.error).to.equal('Login failed. Username or password invalid.');
          done();
        });
    });
  });
  ...
  describe('Create user', function () {
    ...
    it('should validate that provided user data is valid', function (done) {
      ...
      expect(bcrypt.compareSync(userOne.password, resData.body.user.password)).to.be.true;
  ...
  describe('Get users', function () {
    it('should return the user if userid matches tokenid', function (done) {
    ...
    it('should return an error if user is not registered', function (done) {
      api.get('/users/:id').expect(401).end((err, res) => {
        if (err) {
          return done(err);
        }
        expect(res.body).to.have.property('error');
  ...
  describe('Get user documents', function () {
    it('should return all user documents for admins and owners', function (done) {
    ...
    it('should return only users public documents for guests and other users', function (done) {
  ...
  describe('Update user', function () {
    ...
    it('should update user if tokenid matches user or admin', function (done) {
    ...
    it('should return an error if tokenid does not match user or admin', function (done) {
      ...
      api.put('/users/7').set({ 'x-access-token': userToken })
        ...
        .expect(401).end((err, res) => {
          ...
          expect(res.body).to.have.property('error');
          expect(res.body.error).to.equal('You do not have permission to update user data.');
          done();
        });
    });

    it('should update user role if tokenid matches admin token', function (done) {
      api.put('/users/7').set({ 'x-access-token': adminToken })
    ...
    it('should return an error if tokenid does not match user or admin', function (done) {
      ...
      api.delete('/users/10').set({ 'x-access-token': userToken })
        .expect(401).end((err, res) => {
          ...
          expect(res.body).to.have.property('error');
          expect(res.body.error).to.equal('You do not have permission to delete user.');
          done();
        });
    });
  })
